...
define('payzippy_imgdir', WP_PLUGIN_URL . "/" . plugin_basename(dirname(__FILE__)) . '/assets/img/');

function woocommerce_payzippy_init(){
	if(!class_exists('WC_Payment_Gateway')) return;

    if( isset($_GET['msg']) && !empty($_GET['msg']) ){
        add_action('the_content', 'payzippy_showMessage');
    }
    function payzippy_showMessage($content){
	...
	class WC_payzippy extends WC_Payment_Gateway{
		public function __construct(){
			$this->id 					= 'payzippy';
			$this->icon         		= payzippy_imgdir . 'logo.gif';
			...
			$this->form_fields = array(
				'enabled' => array(
      			...
      			'redirect_page_id' => array(
					...
					'type' 			=> 'select',
					'options' 		=> $this->payzippy_get_pages('Select Page'),
		...
		public function admin_options(){
			...
			echo '<p><strong style="color:red">'.__('PayZippy is currently not accepting any new subscription!').'</strong></p>';
				...
				if($order_id != ''){
					try{
						...
						if( $order->status !=='completed' ){
							...
							//add_action('the_content', array(&$this, 'payzippy_showMessage'));
						}
					}catch(Exception $e){
                        // $errorOccurred = true;
                        $msg = "Error";
		...
		*/
		...
		// get all pages
		function payzippy_get_pages($title = false, $indent = true) {
